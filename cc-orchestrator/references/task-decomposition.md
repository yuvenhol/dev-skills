...
```
[architect]
    ↓ file: _workspace/01_architect_design.md
[developer]
    ↓ file: code + _workspace/02_developer_changelog.md
    ↓ message: SendMessage to notify the reviewer of the review scope
[reviewer]
    ↓ file: _workspace/03_reviewer_report.md
    ↓ message: SendMessage review comments to the developer
[developer]
    ↓ revise the code
[tester]
    ↓ file: test code + _workspace/04_tester_report.md
    ↓ message: SendMessage bug reports to the developer
```

**Basis for choosing the passing method:**
- Content > 100 lines → file.
- Real-time coordination / lightweight feedback → SendMessage.
- Progress and dependency management → TaskCreate/TaskUpdate.
- Sub-agent results → return value.
